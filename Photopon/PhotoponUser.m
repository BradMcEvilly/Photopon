...
@implementation PhotoponUser

static PhotoponUser* current = nil;
...
+ (PhotoponUser *)currentUser{
    ...
    if(userId){
        ...
        if(!error && [array count] > 0)
            current = (PhotoponUser*)[array objectAtIndex:0];    
    }
    return current;
}
...
+ (PhotoponUser *)user{
	return (PhotoponUser*)[PhotoponUser objectWithClassName: kUserClassName];
}
