...
@interface PhotoponUIWebViewController ()
...
@implementation PhotoponUIWebViewController
...
- (void)refreshWebView {
    ...
    if (![[PhotoponAppDelegate sharedPhotoponApplicationDelegate] validateDeviceConnection])
        return;
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    ...
    if (![requestedURL isEqual:self.url] &&
        ...
        PhotoponUIWebViewController *webViewController = [[PhotoponUIWebViewController alloc] init];
...
-(void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
	...
	if (buttonIndex == 0) {
        ...
        if (controller)
            ...
            //[self.navigationController.presentedViewController presentSemiViewController:controller];
