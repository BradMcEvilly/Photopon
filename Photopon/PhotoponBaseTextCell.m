...
@interface PhotoponBaseTextCell () {
...
@implementation PhotoponBaseTextCell
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        horizontalTextSpace =  [PhotoponBaseTextCell horizontalTextSpaceForInsetWidth:cellInsetWidth];
...
+ (CGFloat)heightForCellWithName:(NSString *)name contentString:(NSString *)content {
    ...
    return [PhotoponBaseTextCell heightForCellWithName:name contentString:content cellInsetWidth:0];
}
...
+ (CGFloat)heightForCellWithName:(NSString *)name contentString:(NSString *)content cellInsetWidth:(CGFloat)cellInset {
    ...
    NSString *paddedString = [PhotoponBaseTextCell padString:content withFont:[UIFont systemFontOfSize:13] toWidth:nameSize.width];
    CGFloat horizontalTextSpace = [PhotoponBaseTextCell horizontalTextSpaceForInsetWidth:cellInset];
...
- (void)setUser:(PhotoponUserModel *)aUser {
    ...
    //[self.avatarImageView setFile:self.user.profilePictureUrl];
    [self.nameButton setTitle:self.user.fullname forState:UIControlStateNormal];
    [self.nameButton setTitle:self.user.fullname forState:UIControlStateHighlighted];
...
- (void)setContentText:(NSString *)contentString {
    ...
    if (self.user) {
        CGSize nameSize = [self.nameButton.titleLabel.text sizeWithFont:[UIFont boldSystemFontOfSize:13] forWidth:nameMaxWidth lineBreakMode:UILineBreakModeTailTruncation];
        NSString *paddedString = [PhotoponBaseTextCell padString:contentString withFont:[UIFont systemFontOfSize:13] toWidth:nameSize.width];
...
- (void)setCellInsetWidth:(CGFloat)insetWidth {
    ...
    horizontalTextSpace = [PhotoponBaseTextCell horizontalTextSpaceForInsetWidth:insetWidth];
    [self setNeedsDisplay];
}
...
- (id<PhotoponBaseTextCellDelegate>)delegate {
    
    NSLog(@"||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||");
    NSLog(@"%@ :: %@            BEGIN BEGIN BEGIN", self, NSStringFromSelector(_cmd));
    NSLog(@"||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||");
    
    return (id<PhotoponBaseTextCellDelegate>)delegate;
}

- (void)setDelegate:(id<PhotoponBaseTextCellDelegate>)aDelegate {
