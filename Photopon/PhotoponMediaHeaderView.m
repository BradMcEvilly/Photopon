...
#import "PhotoponMediaHeaderView.h"
#import "PhotoponProfileImageView.h"
//#import "TTTTimeIntervalFormatter.h"
#import "PhotoponUtility.h"

@interface PhotoponMediaHeaderView ()
...
@implementation PhotoponMediaHeaderView
...
- (id)initWithFrame:(CGRect)frame buttons:(PAPPhotoHeaderButtons)otherButtons {
    ...
    if (self) {
        [PhotoponMediaHeaderView validateButtons:otherButtons];
        ...
        // translucent portion
        self.containerView = [[UIView alloc] initWithFrame:CGRectMake( 6.0f, 0.0f, self.bounds.size.width - 6.0f * 2.0f, self.bounds.size.height)];
        [self addSubview:self.containerView];
        [self.containerView setBackgroundColor:[UIColor colorWithPatternImage:[UIImage imageNamed:@"PhotoponBackgroundComments.png"]]];
...
- (void)setPhoto:(PhotoponMediaModel *)aPhoto {
    ...
    // user's avatar
    PhotoponUserModel *user = [self.photo objectForKey:kPhotoponMediaAttributesUsersKey];
    PhotoponFile *profilePictureSmall = [user objectForKey:kPhotoponUserAttributesProfilePictureUrlKey];
    //[self.avatarImageView setFile:profilePictureSmall];
    
    NSString *authorName = [user objectForKey:kPhotoponUserAttributesFullNameKey];
...
- (void)didTapUserButtonAction:(UIButton *)sender {
    if (delegate && [delegate respondsToSelector:@selector(photoHeaderView:didTapUserButton:user:)]) {
        [delegate photoHeaderView:self didTapUserButton:sender user:[self.photo objectForKey:kPhotoponMediaAttributesUsersKey]];
    }
}
